...
const zend_function_entry spl_ds_methods_Queue[] = {
    ...
    SPL_DS_ME(DLL,   rewind,  spl_ds_arg_info_Queue_void)
    SPL_DS_ME(DLL,   current, spl_ds_arg_info_Queue_void)
    SPL_DS_ME(DLL,   key,     spl_ds_arg_info_Queue_void)
    SPL_DS_ME(DLL,   next,    spl_ds_arg_info_Queue_void)
    SPL_DS_ME(DLL,   valid,   spl_ds_arg_info_Queue_void)
    PHP_FE_END
};
...
PHP_MINIT_FUNCTION(spl_ds_queue)
{
    ...
    zend_class_implements(spl_ds_ce_Queue TSRMLS_CC, 2,
        zend_ce_iterator,
